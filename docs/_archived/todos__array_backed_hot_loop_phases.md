...
**Status**: Phases 1-4 ✅ COMPLETE; Phase 5 🔴 BLOCKED (P0 input-source routing fix required)  
**Created**: 2025-12-16  
...
- Audits wired via CLI:
  ...
  - Phase 2 in-memory parity: `trade_cli.py backtest phase2-audit`
...
- [x] 2.2 Indicator math parity gate (in-memory parity; no artifacts)
  ...
  - CLI: `trade_cli.py backtest phase2-audit --idea-card <path> --start <date> --end <date>`
...
```
trade_cli.py backtest phase2-audit --idea-card BTCUSDT_15m_mtf_tradeproof --start "2025-12-01" --end "2025-12-14"
...
| `src/tools/backtest_cli_wrapper.py` | 2 | Add `phase2-audit` gate tool |
| `trade_cli.py` | 2 | Add `backtest phase2-audit` command |
...
- [x] 4.5 Validate acceptance gate
  ...
  - Run: `python trade_cli.py backtest phase2-audit` ✅ PASSED (contract=42/42, parity=3/3)
...
**Phase 5 (Market Structure Features) is BLOCKED** until the input-source routing bug is fixed.

### Issue Summary

**Bug Location**: `src/backtest/features/feature_frame_builder.py` (lines 633, 674)

**Problem**: When `input_source != "close"`, the feature builder retrieves the correct input series but then **ignores it** and passes close prices instead:
...
**Impact**:
- All indicators with `input_source != "close"` compute on wrong data
- Volume SMA shows 102K discrepancy vs pandas_ta
- Affects: volume, open, high, low, hlc3, ohlc4 input sources
- **ROOT CAUSE DIAGNOSED**: Volume SMA failure was plumbing bug, NOT missing registry entry

**Required Before Phase 5**:
1. Fix lines 633 and 674 in `feature_frame_builder.py`
2. Run input-source parity sweep:
   - Test SMA on volume (currently failing)
   - Test SMA on open/high/low
   - Test indicators on hlc3, ohlc4
3. All input-source parity tests must PASS

**See**:
- `docs/audits/volume_sma_bug_diagnosis.md` — Complete diagnostic report
- `docs/reviews/INDICATOR_PIPELINE_END_TO_END_REVIEW.md` — Pipeline review
- `docs/audits/math_parity_5m_stress_test.md` — Math parity audit results

**Gate**: Input-source fix + parity sweep passing → Phase 5 unblocked

---

## Phase 5 — Market Structure Features (BLOCKED)

**Status**: 🔴 **BLOCKED** — Awaiting P0 input-source routing fix  
**Goal**: Add market structure feature layer (swings, pivots, trends, regimes)

This phase is deferred until input-source routing is fixed and validated.

**Planned Scope** (Once Unblocked):
...
**NOT starting Phase 5 until**:
- ✅ Input-source routing fix applied
- ✅ Input-source parity sweep passes (volume, open, high, low, hlc3, ohlc4)
- ✅ Math parity audit shows 0 failures
