...
def validate_data_quality_tool(
    symbol: str | None = None,
    env: DataEnv = DEFAULT_DATA_ENV,
) -> ToolResult:
    """
    G3-3: Validate data quality for symbol(s) in the database.

    Detects data quality issues:
    - Stuck prices (>10 consecutive bars with identical close)
    - Volume spikes (>10x median volume)
    - Uniform data (identical OHLCV across many bars)
    ...
    Returns:
        ToolResult with validation results per symbol/timeframe
    """
    try:
        from src.data.historical_maintenance import validate_data_quality
        ...
        if not symbols:
            return ToolResult(
                success=True,
                message=f"[{env.upper()}] No symbols in database to validate",
                data={"validation": {}, "env": env},
                source="duckdb",
            )
        ...
        for sym in symbols:
            ...
            for tf in timeframes:
                key = f"{sym}_{tf}"
                result = validate_data_quality(store, sym, tf)
        ...
        if total_issues == 0:
            message = f"[{env.upper()}] Data quality validation passed for {len(symbols)} symbol(s) - no issues found"
        else:
            message = f"[{env.upper()}] Data quality validation found {total_issues} issue(s) across {len(all_results)} symbol/timeframe(s)"
        ...
        return ToolResult(
            success=False,
            error=f"Failed to validate data quality: {str(e)}",
        )
