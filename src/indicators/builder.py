...
Design principles:
- All computation outside hot loop (vectorized via vendor)
...
from .vendor import compute_indicator
...
class IndicatorCompute:
    ...
    **ALL pandas_ta indicators are available dynamically!**
    ...
    Note: This is distinct from IndicatorRegistry in registry.py,
    ...
    def has(self, indicator_type: str) -> bool:
        ...
        Validates against the registry module for supported types.
        """
        from .registry import get_registry as get_main_registry
...
class FeatureFrameBuilder:
    ...
    Performance:
        - All computation is vectorized (via vendor)
    ...
    @property
    def registry(self) -> IndicatorCompute:
        """Alias for compute adapter (backward compatibility)."""
        return self.compute
