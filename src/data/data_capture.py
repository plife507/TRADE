...
class DataCapture:
    ...
    def __init__(self):
        ...
        # ALWAYS use LIVE API for data capture (for accuracy)
        # Captured data must be accurate - DEMO API may have different/simulated data
        ...
        if not data_key or not data_secret:
            self.logger.warning(
                ...
                "Data capture requires LIVE API access for accurate data. "
                "Set BYBIT_LIVE_DATA_API_KEY/SECRET or BYBIT_LIVE_API_KEY/SECRET."
            )
        
        # Initialize client with LIVE API (use_demo=False)
        # This ensures we capture real market data regardless of trading mode
        ...
        self.logger.info(
            "DataCapture initialized with LIVE API (api.bybit.com) for accurate data"
        )
