...
Level Calculation (Unified Formula):
    All levels use: level = high - (ratio × range)
...
class IncrementalFibonacci(BaseIncrementalDetector):
    ...
    # - retracement: level = high - (ratio × range) - works with any ratio
    # - extension: auto-direction based on pair_direction (requires use_paired_anchor)
    # - extension_up: (legacy) level = high + (ratio × range)
    # - extension_down: (legacy) level = low - (ratio × range)
    ...
    def _validate_params(
        ...
        if mode not in cls.VALID_MODES:
            raise ValueError(
                ...
                f"\n"
                f"Fix:\n"
                f"  mode: retracement   # Unified formula: level = high - (ratio × range)\n"
                f"  mode: extension     # Auto-direction based on pair (requires use_paired_anchor)\n"
                f"  mode: extension_up  # (legacy) level = high + (ratio × range)\n"
                f"  mode: extension_down  # (legacy) level = low - (ratio × range)"
            )
        ...
        for lvl in self.levels:
            ...
            if self.mode == "retracement":
                ...
                if direction == "bullish":
                    ...
                    # Input ratio 0.272 → actual -0.272 → high + 0.272 × range
                    self._values[key] = high + (range_val * lvl)
                elif direction == "bearish":
                    # Short targets: below low
                    # Input ratio 0.272 → actual 1.272 → low - 0.272 × range
