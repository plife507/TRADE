...
Checks:
1. slippage_bps: IdeaCard → ExecutionConfig
2. taker_fee_rate: IdeaCard → RiskProfileConfig (fees flow to risk profile, not execution)
3. maker_fee_bps: IdeaCard → strategy_params (for future limit order support)
4. maintenance_margin_rate: IdeaCard → RiskProfileConfig
5. min_trade_notional_usdt: IdeaCard → RiskProfileConfig
"""
...
def audit_value_flow(
    ...
    """
    Audit that IdeaCard values flow correctly to engine configuration.
    ...
    slippage_check = ValueFlowCheck(
        ...
        source="IdeaCard.account.slippage_bps",
    ...
    if idea_card.account.fee_model:
        ...
        taker_check = ValueFlowCheck(
            ...
            source="IdeaCard.account.fee_model.taker_rate",
            target="engine.config.risk_profile.taker_fee_rate",
        )
        checks.append(taker_check)
        if not taker_check.passed:
            errors.append(
                f"taker_fee_rate mismatch: IdeaCard={expected_taker_rate}, Engine={actual_taker_rate}"
            )
    ...
    if idea_card.account.fee_model:
        ...
        maker_check = ValueFlowCheck(
            ...
            source="IdeaCard.account.fee_model.maker_bps",
            target="engine.config.params['maker_fee_bps']",
        )
        checks.append(maker_check)
        if not maker_check.passed:
            errors.append(
                f"maker_fee_bps mismatch: IdeaCard={expected_maker}, Engine={actual_maker}"
            )
    ...
    mmr_check = ValueFlowCheck(
        ...
        source="IdeaCard.account.maintenance_margin_rate",
        target="engine.config.risk_profile.maintenance_margin_rate",
    )
    checks.append(mmr_check)
    if not mmr_check.passed:
        errors.append(
            f"maintenance_margin_rate mismatch: IdeaCard={expected_mmr}, Engine={actual_mmr}"
        )
    ...
    if idea_card.account.min_trade_notional_usdt is not None:
        ...
        min_trade_check = ValueFlowCheck(
            ...
            source="IdeaCard.account.min_trade_notional_usdt",
            target="engine.config.risk_profile.min_trade_usdt",
        )
        checks.append(min_trade_check)
        if not min_trade_check.passed:
            errors.append(
                f"min_trade_notional_usdt mismatch: IdeaCard={expected_min_trade}, Engine={actual_min_trade}"
            )
