...
def test_raw_signature():
    ...
    assert api_key == api_key.strip(), "API key has leading/trailing whitespace!"
    assert api_secret == api_secret.strip(), "API secret has leading/trailing whitespace!"
...
def test_pybit_direct():
    ...
    assert server_time.get("retCode") == 0, f"Server time failed: {server_time}"
    
    # Test authenticated endpoint
    print("\nTesting authenticated endpoint (wallet balance)...")
    balance = session.get_wallet_balance(accountType="UNIFIED")
    
    assert balance.get("retCode") == 0, f"Wallet balance failed: {balance.get('retMsg')}"
...
def test_with_stripped_keys():
    ...
    # Keys should not have hidden whitespace
    assert len(api_key) == len(api_key_clean), "Key had hidden whitespace characters!"
    assert len(api_secret) == len(api_secret_clean), "Secret had hidden whitespace characters!"
    ...
    assert balance.get("retCode") == 0, f"Failed with cleaned keys: {balance.get('retMsg')}"
...
def test_key_permissions():
    ...
    assert key_info.get("retCode") == 0, f"Failed to get key info: {key_info.get('retMsg')}"
